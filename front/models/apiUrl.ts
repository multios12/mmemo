...
export const getMonthApi = (month: string) => {
  let url = `${apiUrl}/api/diary/${month}`;
  return fetch(url)
}
export const getDetailApi = (year: string, month: string, day: string) => {
  let url = `${apiUrl}/api/diary/${year}/${month}/${day}`;
  return fetch(url)
}
